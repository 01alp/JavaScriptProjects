...
function generateMap(map) {
  const game = document.querySelector('.mapsContainer');
  game.innerHTML = '';
...
function update(e) {
  if (!GAME_STATE.isPaused) {
    ...
    if (playerHasWon()) {
      ...
      while (true) {
        name = prompt('Congrats Pilot! You passed the test! Enter your name(only letters and numbers!):');
        //Check name
      ...
      saveUserScore(name, Math.floor((GAME_STATE.score / timePassed.toFixed(2)) * 100) + 10000);
      timePassed = 0;
      return;
    }
